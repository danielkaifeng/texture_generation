...
def Upsample(inputs, filters, kernel_size=3):
...
def Upsample2(inputs, filters, kernel_size=3):
    new_H, new_W = 2 * tf.shape(inputs)[1], 2 * tf.shape(inputs)[2]
    inputs = tf.image.resize_images(inputs, [new_H, new_W])

    return Conv2DNormLReLU(inputs, 128, kernel_size=kernel_size)
    ...
    def __init__(self, inputs):
        with tf.variable_scope('generator'):
            ...
            with tf.variable_scope('u1'):
                inputs = Upsample(inputs,128)    # The number of the filters in this layer is 128 while it is 64 in the graph of the paper. Please refer to the code.
                inputs = Conv2DNormLReLU(inputs, 64)
                inputs = Conv2DNormLReLU(inputs, 64)
            """
