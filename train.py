...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        while step < epochs:
            ...
            #label += 0.00001 * (np.random.random((label.shape)) - 0.5) 
            ...
            for n in range(d_iter):
                sess.run(model.D_opt, feed_dict={model.z: z,  model.x: x, model.label: label, model.dropout: dropout, model.is_train: True})
            ...
            for n in range(g_iter):
                sess.run(model.G_opt, feed_dict={model.z: z, model.x: x, model.label: label, model.dropout: dropout, model.is_train: True})
            ...
            if step % 10 == 0: 
                ...
                g_loss, d_loss = sess.run([model.G_loss, model.D_loss], feed_dict={model.z: z, model.x: x, model.label: label, model.dropout: dropout, model.is_train: False})
                print("Epoch %d/%d - G/D loss: %.4f\t%.4f" % (step+1, epochs, g_loss, d_loss))

                if step > 10000 and step % 500 == 0:    
                ...
                if step > 10000 and step % 100 == 0:
                    ...
                    #g_sample = sess.run(model.logit2, feed_dict={model.z: z[:4], model.dropout: 0, model.is_train: False})
                    encode_sample = sess.run(model.logits, feed_dict={model.x: x, model.dropout: 0, model.is_train: False})
                    ...
                    """
                    for ii, val in enumerate(encode_sample):
                        img_arr = (val+1) * 128 
                        im = Image.fromarray(img_arr.astype('uint8'))
                        im.save("%s/%d_encode.jpg" % (output_dir, ii))
